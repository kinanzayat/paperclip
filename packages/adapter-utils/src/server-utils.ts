...
export function renderPaperclipWakePrompt(
  ...
  const lines = resumedSession
      ? [
        ...
        "",
        "Resume the current issue session and handle only the wake delta below.",
        "Fetch the full API thread only when `fallbackFetchNeeded` is true or the inline batch is not enough.",
        "",
    ...
    : [
        ...
        "Use the inline batch first; fetch the full thread only when `fallbackFetchNeeded` is true or you need broader history.",
        "",
        `- reason: ${normalized.reason ?? "unknown"}`,
        `- issue: ${normalized.issue?.identifier ?? normalized.issue?.id ?? "unknown"}${normalized.issue?.title ? ` ${normalized.issue.title}` : ""}`,
        `- pending comments: ${normalized.includedCount}/${normalized.requestedCount}`,
